...
export default class HDKey {
  ...
  //extended versions
    ...
    //   if (this._privateKey&& this.privateKey) return bs58check.encode(serialize(this, this.versions.private, Buffer.concat([Buffer.alloc(1, 0), this.privateKey])))
    //   else return null
    // }
    ...
    private deriveChild (index:number):HDKey {
      ...
      if (isHardened) { // Hardened child
        ...
        if (this.privateKey){
          ...
          let zb = Buffer.alloc(1)
          let pkConcat = Buffer.concat([ zb, pk])
    ...
    public static fromMasterSeed = function (seedBuffer:crypto.BinaryLike, versions?:Versions) {
      ...
      var hdkey =versions? new HDKey( versions):new HDKey()
