...
import { KEY_LENGTH, SEC_LENGTH, NONCE_LENGTH, PKCS8_DIVIDER, PKCS8_HEADER } from './defaults';

const SEED_OFFSET = PKCS8_HEADER.length;
const DIV_OFFSET = SEED_OFFSET + KEY_LENGTH;
const PUBLIC_OFFSET = SEED_OFFSET + KEY_LENGTH + PKCS8_DIVIDER.length;

export default function decode (version: KeyringPair$JsonVersion, passphrase?: string, _encrypted?: Uint8Array) {
  ...
  const seed = version === '0'
    ? encoded.subarray(SEED_OFFSET, SEED_OFFSET + KEY_LENGTH)
    : encoded.subarray(SEED_OFFSET, SEED_OFFSET + SEC_LENGTH);
  const secretKey = version === '0'
    ? u8aConcat(seed, publicKey)
    : seed;
