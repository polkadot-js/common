...
import { _n, createView, Hash, Input, toBytes } from './utils';
...
export abstract class SHA2<T extends SHA2<T>> extends Hash<T> {
  ...
  update (data: Input): this {
    ...
    const { blockLen, buffer, finished, view } = this;
  ...
  digestInto (out: Uint8Array) {
    ...
    const { blockLen, buffer, isLE, view } = this;
    let { pos } = this;
  ...
  _cloneInto (to?: T): T {
    ...
    to.set(...this.get());
    const { blockLen, buffer, destroyed, finished, length, pos } = this;
