...
import { assertNumber, HashXOF, Input, toBytes, u8, u32, wrapConstructorWithOpts } from './utils';
...
class BLAKE3 extends blake2.BLAKE2<BLAKE3> implements HashXOF<BLAKE3> {
  ...
  protected set () {}
  private b2Compress (counter: number, flags: number, buf: Uint32Array, bufPos = 0) {
    const { pos, state } = this;
  ...
  protected compress (buf: Uint32Array, bufPos = 0, isLast = false) {
    // Compress last block
  ...
  override _cloneInto (to?: BLAKE3): BLAKE3 {
    ...
    const { IV, chunkOut, chunkPos, chunksDone, flags, posOut, stack, state } = this;
  ...
  destroy () {
    ...
    for (const i of this.stack) i.fill(0);
  }

  // Same as b2Compress, but doesn't modify state and returns 16 u32 array (instead of 8)
  private b2CompressOut () {
    const { buffer32, bufferOut32, flags, pos, state } = this;
