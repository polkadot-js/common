...
import { _n, assertNumber, Hash, HashXOF, Input, toBytes, u32, wrapConstructor, wrapConstructorWithOpts } from './utils';
...
export function keccakP (s: Uint32Array, rounds = 24) {
...
export class Keccak extends Hash<Keccak> implements HashXOF<Keccak> {
  ...
  constructor (
    ...
    if (this.blockLen <= 0 || this.blockLen >= 200) { throw new Error('Sha3 supports only keccak-f1600 function'); }
  ...
  _cloneInto (to?: Keccak): Keccak {
    const { blockLen, enableXOF, outputLen, rounds, suffix } = this;
